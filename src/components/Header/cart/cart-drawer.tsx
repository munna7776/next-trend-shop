...
const CartDrawer = ({ onClick, cart }: { 
  onClick: () => void;
  cart: Cart; 
}) => {
  return (
    <>
      <div onClick={onClick} className="absolute left-0 top-0 overflow-hidden w-screen h-screen z-[9999] bg-[#0003]" />
        ...
        </div>
        {
          cart.lines.length === 0 && (
            <div className="mt-8 flex flex-col items-center" >
              ...
              <h2 className="text-lg my-8 font-semibold text-[#3c3c3c]">Your cart is empty</h2>
              <button className="block w-full text-xl p-4 rounded-md my-3 bg-black text-white" >Continue Shopping</button>
            </div>
          )
        }
      </div>
