    ...
    return (
      <div className="border p-6 bg-[#f7f7f9] rounded-lg" >
        ...
        <div className="flex flex-wrap items-center gap-4">
            <button className="border border-black rounded-md px-5 py-1 flex gap-2 items-center" onClick={() => setShowDrawer(true)} >
                <span>Edit</span>
                <EditIcon />
            </button>
            <button disabled={pending} onClick={handleDeleteAddress} className="border border-black rounded-md px-5 py-1 flex gap-2 items-center">
                <span>{pending ? "Removing" : "Remove"}</span>
                <DeleteIcon className={pending ? "pulse" : ""} height={16} width={16} />
            </button>
        </div>
        {
          showDrawer && (
